...
- [x] TASK-013: AGENTS.md creation for Golem itself

## Test Results
- **63 tests, 0 failures, 0 skipped**
- ToolRegistryTest: 4 tests ✅
- GolemAgentTest: 5 tests ✅ (ReAct loop, tool calls, maxTurns, errors, parallel tools)
...
- GolemMcpServerTest: 7 tests ✅ (server build, all types, tool registry, full tools)
- GolemCronParserTest: 3 tests ✅ (parse, blank, 6-field reject)
- GolemJobStoreTest: 5 tests ✅ (add, list, getDueJobs, updateNextRun, remove)
- GolemSchedulerTest: 4 tests ✅ (pollOnce, skip non-due, fail-safe, multiple jobs)
- GolemApiServerTest: 3 tests ✅ (health, agent run, jobs CRUD)
- SchedulerToolsTest: 3 tests ✅ (scheduler_add, scheduler_list, scheduler_remove)

## Epic-005: MCP Server ✅ (Task #2)
- [x] TASK-0014: GolemMcpServer class (tool → MCP tool mapping with JSON Schema)
...
- [x] TASK-020: GolemCronParser (5-field → TramAI CronSchedule wrapper)
- [x] TASK-021: GolemJobStore (SQLite + Exposed, CRUD, getDueJobs)
- [x] TASK-022: GolemScheduler (polling daemon, coroutine-based)
...
- [x] TASK-025: GolemApiServer (Ktor, :8082, /api/health, /api/agent/run)
...
- [x] TASK-030: SchedulerTools.kt (scheduler_add/list/remove Golem tools)
- [x] TASK-031: MCP registration (scheduler tools in McpRunner.kt)
- [x] TASK-032: GolemFactory integration (scheduler tools when schedulerDbPath set)
...
- [x] T-404: Application Metrics (Prometheus, /api/metrics, GolemMetrics)
...
- [x] GolemFactory.kt integration: wire skill system into agent
...
- [x] Phase 0.2: GolemConfigFileStore + ConfigRoutes for YAML persistence
...
- [x] Phase 2.B: Settings tab — real Save button writing to ~/.golem/config.yaml

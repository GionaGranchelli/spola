...
## Background
Golem already has multi-platform delivery tools (telegram_send, email_send). A TTS tool lets the agent speak responses aloud or send voice messages via delivery channels (e.g. Telegram voice messages).
...
- Parameters:
  ...
  - `output_path` (string, optional) — custom file path for the audio (default: `~/.golem/audio/<md5>.wav`)
- Returns: path to the generated audio file, duration info
- Saves WAV/MP3 to `~/.golem/audio/` directory (auto-create)
- Caches: if same text was already synthesized, returns cached file path (MD5 of text + voice)

### 4. Configuration

```kotlin
data class GolemTtsConfig(
    ...
    val cacheDir: String = "~/.golem/audio",
)
```
...
### 5. Integration
- GolemFactory: create TtsProvider, register `tts_say` tool when config has TTS enabled
- Register in `GolemFactory.create()` next to delivery tools
...
golem-core/src/main/kotlin/dev/golem/
├── GolemFactory.kt               — MODIFY: wire TTS provider + tool
├── GolemConfig.kt                — MODIFY: add tts config fields

golem-core/src/test/kotlin/dev/golem/tts/

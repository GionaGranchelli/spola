...
**Goal:** Ship a secure, honest, focused Golem — fix the 3-review consensus findings, then narrow to the JVM moat.
...
|---|---------|--------|-------|----------|
| 1 | Missing tool registrations in `GolemFactory.create()` (AgentTools, SkillTools, DeliveryTools) | Codex+Copilot+Gemini | P1 | CRITICAL |
...
| 10 | GolemFactory.kt (534 lines) is kitchen-sink — 2 creation paths that drifted apart | Copilot+Gemini | P2 | MEDIUM |
| 11 | ArchitectMode duplicates provider resolution from GolemFactory | Copilot+Gemini | P2 | MEDIUM |
| 12 | API server tool registry independent from GolemFactory — must manually sync | Copilot+Gemini | P2 | MEDIUM |
...
**File:** `golem-cli/src/main/kotlin/dev/golem/cli/Main.kt`
**File:** `golem-core/src/main/kotlin/dev/golem/api/GolemApiServer.kt`
**File:** `golem-core/src/main/kotlin/dev/golem/api/ApiAuth.kt`

- Change `GolemApiServer` to bind to `127.0.0.1` by default instead of `0.0.0.0`
...
**File:** `golem-core/src/main/kotlin/dev/golem/GolemFactory.kt`
...
**File:** `golem-core/src/main/kotlin/dev/golem/mcp/McpRunner.kt`
- Add `AgentTools` and `SkillTools` to MCP server tool registry

**Tests:**
- Add `GolemFactoryToolRegistrationTest` — verify `create()` and `createFromAgentDefinition()` both produce registries containing all expected tool families
- Add `ApiToolRegistryTest` — verify API server tool registry matches GolemFactory's
...
**File:** `golem-core/src/main/kotlin/dev/golem/GolemAgent.kt`
- In `callLlm()`: pass `config.temperature` and `config.maxTokens` to `ModelRequest`
- These fields already exist on `AgentDefinition` and are parsed by `GolemFactory` — they just never make the last hop

**Tests:**
- Add `GolemAgentModelParametersTest` — create agent with known temp/maxTokens, verify `ModelRequest` contains them

**Review findings closed:** #2 (temperature/maxTokens silently ignored)

### Fix 1.4: Fix or kill SSE transport

**File:** `golem-core/src/main/kotlin/dev/golem/mcp/McpClientManager.kt` (~line 229)
...
**File:** Create `golem-core/src/test/kotlin/dev/golem/agent/PermissionEnforcerTest.kt`
...
**File:** Update `golem-core/src/main/kotlin/dev/golem/agent/PermissionEnforcer.kt`
...
**File:** Create `golem-cli/src/test/kotlin/dev/golem/cli/MainTest.kt`

Test key codepaths without needing a running agent:
- `GolemCli.call()` for each mode flag (`--api`, `--version`, `--mcp`, `--help`)
...
### Fix 2.2: Refactor GolemFactory → split concerns

**File:** `golem-core/src/main/kotlin/dev/golem/GolemFactory.kt` (534 lines → split)

Split into:
- `AgentFactory` — creates `GolemAgent` instances from `AgentDefinition` or defaults
...
Each class under 150 lines. `GolemFactory` becomes a simple orchestrator calling the 4 factories.

**Note:** `ToolRegistryFactory` as a single source of truth eliminates finding #12 (API server registry drift) permanently.

**Review findings closed:** #10 (GolemFactory kitchen-sink), #12 (API server registry drift)

### Fix 2.3: Deduplicate provider resolution

**File:** `golem-core/src/main/kotlin/dev/golem/ArchitectMode.kt` — remove `resolveNamedProvider()`, reference `GolemFactory` or the new `ProviderResolver` instead.

**Review findings closed:** #11 (ArchitectMode duplication)

### Fix 2.4: search_files streaming

**File:** `golem-core/src/main/kotlin/dev/golem/tools/FileTools.kt` (~line 143-181)
...
**File:** `golem-core/src/main/kotlin/dev/golem/compression/TokenJuice.kt` (~line 59-62)
...
**File:** `golem-core/src/main/kotlin/dev/golem/GolemAgent.kt` (~line 50-62)
**File:** `golem-core/src/main/kotlin/dev/golem/checkpoint/CheckpointManager.kt`
...
**File:** `golem-core/src/main/kotlin/dev/golem/agent/AgentTools.kt` (~line 242-250)

Currently returns `ToolResult.fail("agent_executor not available")`. Either implement it (call the API server's agent run endpoint) or replace with a clear error: "agent_run requires the API server to be running with `golem --api`".

**Review findings closed:** #17 (agent_run stub)

### Fix 3.3: parallelAgentsStep → direct call, not HTTP

**File:** `golem-core/src/main/kotlin/dev/golem/workflow/TeamWorkflowSteps.kt` (~line 75-137)

Replace the HTTP `http://localhost:8082/api/agents/run` call with:
- A direct method call on `GolemAgent` (create agent instance, run, collect result)
...
**File:** `golem-core/src/main/kotlin/dev/golem/api/GolemApiServer.kt` (~line 236-262)
...
Merge into a single `golem.db` with separate tables. Use a shared `Database` connection in `GolemConfig`. Add connection pooling (HikariCP for SQLite, or manually manage a single connection).
...
**File:** `golem-core/src/main/kotlin/dev/golem/tools/FileTools.kt` (~line 22-27)
...
**File:** `golem-core/src/main/kotlin/dev/golem/Tool.kt`
...
**File:** `golem-core/src/main/kotlin/dev/golem/ChatMessage.kt`
...
| MCP client | ⚠️ Re-evaluate | Does Golem need to consume MCP tools or just expose them? |
...
Phase 2 — Code Health & Test Coverage
├── Fix 2.1  CLI tests
├── Fix 2.2  Split GolemFactory → ToolRegistryFactory (eliminates registry drift)
├── Fix 2.3  Deduplicate provider resolution
...
1. `./gradlew :golem-core:test --no-build-cache` ✅
2. `./gradlew :golem-cli:test --no-build-cache` ✅ (after Phase 2.1)

After Phase 1:
- `./golem-cli` (CLI mode) — one-shot prompt, verifies tool registrations

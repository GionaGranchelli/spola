# Golem Dashboard CRUD Plan

Target frontend: `golem-core/src/main/resources/web/index.html` (`1727` lines, vanilla JS SPA)

This revision is based on the actual API server wiring in `golem-core/src/main/kotlin/dev/golem/api/GolemApiServer.kt:132-157`, the route files under `golem-core/src/main/kotlin/dev/golem/api/routes/`, and the DTOs in `golem-core/src/main/kotlin/dev/golem/api/ApiModels.kt`.
...
- `golem-core/src/main/kotlin/dev/golem/api/routes/ToolRoutes.kt`
...
Why first:
- The Settings tab currently calls `GET /api/config` at `index.html:1546`, but no `/api/config` route is registered in `GolemApiServer.kt:144-155`.
- `GolemConfig` is only a data class in `golem-core/src/main/kotlin/dev/golem/GolemConfig.kt:8-60`.
- There is no general Golem YAML config loader/saver in `golem-core`.

Files:
- Create `golem-core/src/main/kotlin/dev/golem/config/GolemConfigFileStore.kt`
- Create `golem-core/src/main/kotlin/dev/golem/api/routes/ConfigRoutes.kt`
- Update `golem-core/src/main/kotlin/dev/golem/api/ApiModels.kt`
- Update `golem-core/src/main/kotlin/dev/golem/api/GolemApiServer.kt`
...
- Add explicit DTOs instead of serializing `GolemConfig` directly
...
```json
{
  ...
  "workingDirectory": ".",
  "personaPath": null,
  "memoryDbPath": "./.golem/memory.db",
  "schedulerDbPath": "./.golem/scheduler.db",
  "kanbanDbPath": "./.golem/kanban.db",
  "checkpointDbPath": "./.golem/checkpoint.db",
  "jvmIndexDbPath": "./.golem/jvm-index.db",
  ...
  "pluginsDir": "...",
  "agentsDir": "...",
  "agentsDbPath": "./.golem/agents.db",
  "defaultAgentId": null,
  "sessionsDbPath": "./.golem/sessions.db",
...
Notes:
- The current Settings UI references keys that do not exist in `GolemConfig`, including `unsafe`, `email.*`, `tts.*`, `architectMode.enabled`, `architectMode.architectModel`, `architectMode.architectProvider`, `architectMode.editorModel`, `architectMode.editorProvider`, and renamed fields like `workdir` instead of `workingDirectory`.
- Phase 1 must normalize the frontend field map to real `GolemConfig` fields before save can work.
...
- No provider routes are registered in `GolemApiServer.kt:144-155`.

Files:
- Create `golem-core/src/main/kotlin/dev/golem/api/routes/ProviderRoutes.kt`
- Update `golem-core/src/main/kotlin/dev/golem/api/ApiModels.kt`
- Update `golem-core/src/main/kotlin/dev/golem/api/GolemApiServer.kt`
- Extend the new config store from `Phase 0.2`
- Inspect `golem-core/src/main/kotlin/dev/golem/factory/ProviderResolver.kt` and `golem-core/src/main/kotlin/dev/golem/agent/ProviderConfig.kt` when implementing, since provider definitions should align with runtime resolution
...
Files:
- `golem-core/src/main/resources/web/index.html`
...
Files:
- Backend: `golem-core/src/main/kotlin/dev/golem/api/ApiModels.kt`
- Backend: `golem-core/src/main/kotlin/dev/golem/api/routes/ToolRoutes.kt`
- Frontend: `golem-core/src/main/resources/web/index.html`
...
- Multiple frontend field keys do not match `GolemConfig`
...
Relevant source:
- `golem-core/src/main/kotlin/dev/golem/skill/SkillCatalog.kt`
- `golem-core/src/main/kotlin/dev/golem/skill/SkillRepository.kt`
- `golem-core/src/main/kotlin/dev/golem/skill/SkillIndexer.kt`
- `golem-core/src/main/kotlin/dev/golem/skill/SkillTools.kt`
- `golem-core/src/main/kotlin/dev/golem/Tool.kt`
...
Relevant source:
- `golem-core/src/main/kotlin/dev/golem/persona/PersonaLoader.kt`
...
4. Phase 1.4 through 1.7 to expose working backend domains already registered in `GolemApiServer`.

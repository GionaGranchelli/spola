...
## Overview
This plan adds four tool areas without reworking Spola's core architecture:
...
- The ReAct loop in `SpolaAgent` is the correct place to pause for clarification before the next LLM call
...
- `spola-backend-core/src/main/kotlin/dev/spola/SpolaConfig.kt`
  Add browser config fields.
- `spola-backend-core/src/main/kotlin/dev/spola/config/ConfigLoader.kt`
  Add `browser` section parsing and merge behavior.
- `spola-backend-core/src/main/kotlin/dev/spola/tools/Tools.kt`
  Register browser tools in default/API/agent tool sets.
- `spola-backend-core/src/main/kotlin/dev/spola/factory/ToolRegistryFactory.kt`
  Ensure browser tools appear in all intended registries, including MCP.
- `spola-backend-core/src/main/kotlin/dev/spola/mcp/SpolaMcpServer.kt`
...
- `spola-backend-core/src/main/kotlin/dev/spola/agent/PermissionEnforcer.kt`
...
browser:
  ...
  screenshot-dir: "~/.spola/browser"
  snapshot-max-length: 20000
```

`SpolaConfig` additions:
...
- `browserScreenshotDir: String = Path.of(System.getProperty("user.home"), ".spola", "browser").toString()`
- `browserSnapshotMaxLength: Int = 20000`

Notes:

- `launchCommand` is optional and should only be used if the host wants Spola to start Chrome itself.
...
1. Add version catalog and `spola-backend-core` dependencies for Ktor client WebSockets.
2. Extend `SpolaConfig` and `ConfigLoader`.
...
- `spola-backend-core/src/main/kotlin/dev/spola/image/ImageGenProvider.kt`
...
- `spola-backend-core/src/main/kotlin/dev/spola/image/OpenAiImageGenProvider.kt`
  Purpose: built-in DALL-E 3 implementation over HTTP.
  Key classes: `OpenAiImageGenProvider`
- `spola-backend-core/src/main/kotlin/dev/spola/image/StabilityImageGenProvider.kt`
  Purpose: built-in Stability AI implementation over HTTP.
  Key classes: `StabilityImageGenProvider`
- `spola-backend-core/src/main/kotlin/dev/spola/image/ImageGenProviderFactory.kt`
  Purpose: resolve default provider from config and allow plugin-provided overrides.
  Key classes: `ImageGenProviderFactory`
- `spola-backend-core/src/main/kotlin/dev/spola/tools/ImageGenTools.kt`
...
- Plugin extension points if needed:
  `spola-backend-core/src/main/kotlin/dev/spola/plugin/...`
...
object ImageGenProviderFactory {
    fun create(config: SpolaConfig): ImageGenProvider
}
```
...
- `spola-backend-core/src/main/kotlin/dev/spola/codeexec/CodeExecConfig.kt`
  Purpose: code execution defaults and policy model.
  Key classes: `CodeExecConfig`
- `spola-backend-core/src/main/kotlin/dev/spola/codeexec/CodeExecSandbox.kt`
...
- `spola-backend-core/src/main/kotlin/dev/spola/tools/CodeExecTools.kt`
...
- `spola-backend-core/src/main/kotlin/dev/spola/tools/ShellTool.kt`
...
- `SpolaConfigFile` currently uses simple scalar/list fields, not arbitrary nested maps in the main class. Add a dedicated `CodeExecConfigFile` data class as done for `email` and `tts`.
...
- `spola-backend-core/src/main/kotlin/dev/spola/clarify/ClarifyStore.kt`
...
- `spola-backend-core/src/main/kotlin/dev/spola/tools/ClarifyTools.kt`
  Purpose: register `clarify`.
  Key functions: `registerClarifyTool`
- `spola-backend-core/src/main/kotlin/dev/spola/api/routes/ClarifyRoutes.kt`
...
- `spola-backend-core/src/main/kotlin/dev/spola/SpolaAgent.kt`
  Integrate session-aware clarify pause into the ReAct loop.
- `spola-backend-core/src/main/kotlin/dev/spola/api/AgentRunHandler.kt`
  Ensure session IDs are propagated into agent runs.
- `spola-backend-core/src/main/kotlin/dev/spola/api/StreamHandler.kt`
  Emit status updates when waiting for clarification.
- `spola-backend-core/src/main/kotlin/dev/spola/Runner.kt`
...
- `spola-backend-core/src/main/kotlin/dev/spola/api/SpolaApiServer.kt`
...
- `spola-backend-core/src/main/kotlin/dev/spola/process/GateDecisionStore.kt`
...
- `clarifyDbPath: String = "./.spola/clarify.db"`
- `clarifyPollIntervalMs: Long = 1000`
- `clarifyDefaultTimeoutSeconds: Long = 3600`

Important integration note:

- The current API run flow does not pass a `sessionId` into `SpolaAgent.run(...)`.
...
2. Pass that session id into `SpolaAgent.run(...)`.
...
3. Make session identity first-class in `AgentRunRequest`, `AgentRunHandler`, and `SpolaAgent`.
4. Register `clarify` in non-MCP registries.
5. Update `SpolaAgent` loop to pause on pending clarification before the next LLM call.
...
```bash
./gradlew :spola-backend-core:test
./gradlew :spola-backend-cli:run --args="'use clarify to ask me one question before proceeding'"
./gradlew :spola-backend-cli:run --args="'generate an image of a red cube'"
```
...
- Clarify depends on stable session identity, but current API agent runs do not propagate session ids into `SpolaAgent.run(...)`. That is the main architectural delta in this plan.

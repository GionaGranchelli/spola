...
4. `SkillLoader.kt` — rewrite: scan `~/.spola/skills/<category>/<name>/SKILL.md` recursively
...
7. `SpolaConfig.kt` — add `skillsDir`, `skillsEnabled`
...
5. `SpolaConfig.kt` — add `skillsDbPath`
...
4. `SpolaAgent.kt` — regenerate tool schemas on next LLM call after registry change

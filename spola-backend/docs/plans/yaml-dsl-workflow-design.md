...
Golem currently defines workflows as hardcoded Kotlin DSL templates in `WorkflowTemplateRegistry`. This design adds a **YAML → DAG compiler** that lets users define workflows as YAML files, compiled to TramAI `Workflow` objects at runtime — no rebuild required.
...
| `GolemState` | `golem-core/.../GolemState.kt` | Data class is sufficient. Add optional `parameters` field. |
| `AsyncWorkflowDispatcher` | `golem-core/.../WorkflowDispatcher.kt` | Still polls SQLite queue and dispatches execution |
| `WorkflowExecutionService` | `golem-core/.../WorkflowExecutionService.kt` | Still resolves template → builds workflow → runs |
| `WorkflowTemplateRegistry` | `golem-core/.../WorkflowTemplateRegistry.kt` | Gets a new source: YAML-compiled templates alongside hardcoded ones |
| `WorkflowFactory` | `golem-core/.../WorkflowFactory.kt` | No changes needed |
| `SqliteWorkflowExecutionStore` | `golem-core/.../WorkflowExecutionStore.kt` | No changes |
| `WorkflowPersistence` / checkpoint | `tramai-orchestration` | YAML workflows get checkpoint/resume for free |
| `GolemWorkflowObserver` | `golem-core/.../GolemWorkflowObserver.kt` | No changes |
| `GolemWorkflowStateCodec` | `golem-core/.../GolemWorkflowStateCodec.kt` | No changes |
...
```yaml
# ~/.golem/workflows/<name>.yaml
# OR <project>/.golem/workflows/<name>.yaml
...
- id: check-retry
  ...
  # Implementation: compiled to a localStep that reads/writes GolemState fields
```
...
[YamlWorkflowParser] ──► YamlWorkflowDefinition (intermediate IR)
    ...
    │
    ▼
[WorkflowDagCompiler]  ──► Workflow<GolemState, String> (TramAI)
...
[WorkflowExecutionService] ──► run / checkpoint / resume
    │  No changes — receives a Workflow<GolemState, String> like any other
```
...
```kotlin
/**
 * Compiles a YamlWorkflowDefinition into a TramAI Workflow<GolemState, String>.
...
class WorkflowDagCompiler(
    ...
    private fun compileStep(
        ...
        builder: WorkflowBuilder<GolemState>,
    )
    
    private fun compileAiStep(def: AiStepDef, resolver: ParamResolver): WorkflowBuilder<GolemState>.() -> Unit
    private fun compileParallelStep(def: ParallelStepDef, resolver: ParamResolver): WorkflowBuilder<GolemState>.() -> Unit
    private fun compileBranchStep(def: BranchStepDef, resolver: ParamResolver): WorkflowBuilder<GolemState>.() -> Unit
    private fun compileGateStep(def: GateStepDef, resolver: ParamResolver): WorkflowBuilder<GolemState>.() -> Unit
    private fun compilePluginStep(def: PluginStepDef, resolver: ParamResolver): WorkflowBuilder<GolemState>.() -> Unit
    private fun compileShellStep(def: ShellStepDef, resolver: ParamResolver): WorkflowBuilder<GolemState>.() -> Unit
    private fun compileHttpStep(def: HttpStepDef, resolver: ParamResolver): WorkflowBuilder<GolemState>.() -> Unit
    private fun compileDelayStep(def: DelayStepDef, resolver: ParamResolver): WorkflowBuilder<GolemState>.() -> Unit
    private fun compileLocalStep(def: LocalStepDef, resolver: ParamResolver): WorkflowBuilder<GolemState>.() -> Unit
}
```
...
class YamlWorkflowLoader {
    companion object {
        ...
        fun defaultSearchPaths(): List<Path> = listOf(
            Path.of(System.getProperty("user.home"), ".golem", "workflows"),
            Path.of(".golem", "workflows"),  // project-local
            Path.of(".golem", "workflows.yaml"),  // single-file alternative
        )
    }
}
```

### 4.3 GolemState Extension

```kotlin
/**
 * Extended GolemState with workflow definition fields.
...
The `WorkflowTemplate` interface and `WorkflowTemplateRegistry.register()` remain unchanged. Existing code calling `registry.resolve("code-review")` gets the same `Workflow<GolemState, String>` regardless of whether the template was hardcoded or YAML-compiled.
...
| State | None — just context injection | `GolemState` with checkpoint/resume |
...
YAML Workflow File
    │
    ▼
Golem DAG Compiler → TramAI Workflow
```
...
| File | Change |
|------|--------|
| `GolemState.kt` | Add `parameters`, `stepResults`, `currentStepId` fields |
...
| `GolemFactory.kt` | Call `YamlWorkflowLoader.loadAndRegister()` at startup |

### 11.3 Example YAML Workflow Files

| File | Purpose |
|------|---------|
| `~/.golem/workflows/code-review.yaml` | Port of built-in code-review |
| `~/.golem/workflows/jvm-debug.yaml` | Port of built-in jvm-debug |
| `~/.golem/workflows/jvm-refactor.yaml` | Port of built-in jvm-refactor |
| `~/.golem/workflows/jvm-migration.yaml` | Port of built-in jvm-migration |
...
### Day 3 — Done Conditions + GolemState Extension

1. Create `DoneEvaluator.kt` — tier 1/2/3 verification
2. Extend `GolemState.kt` — add `parameters`, `stepResults`, `currentStepId`
...
3. Wire into `GolemFactory.kt` startup
...
params:
  project:
    type: string
    required: true
    description: "Gradle project path (e.g., :golem-core)"
  max_retries:
    type: number
    default: 3

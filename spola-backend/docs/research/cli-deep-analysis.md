# Golem CLI — Deep Technical Analysis

**Date:** 2026-05-14
**Analysts:** OpenAI Codex CLI, GitHub Copilot CLI
**Scope:** golem-cli module + core REPL + provider system (15 source files)
...
The agent's in-memory conversation (`GolemAgent` has a `mutableListOf<ChatMessage>` that accumulates messages) is never reset. The message is a lie.

**Fix:** Add `fun clearConversation()` to `GolemAgent.kt` and call it here.
...
trimmed.startsWith("/provider ") -> {
    ...
    instance = GolemFactory.create(config = currentConfig)  // Can throw!
    // If create() throws, 'instance' now holds a CLOSED reference
    // The 'finally' block tries to close it again — double-close
}
```

`GolemFactory.create()` calls `ProviderStore.get(providerName)`, which calls `ProviderStore.resolve()`. For unknown providers, this throws:
```
...
**Fix:** Wrap `GolemFactory.create()` in a try/catch. On failure, restore the old config and `currentConfig` and print an error. Do NOT close the old instance until the new one is ready.
...
GolemFactory.create(config)
  → AgentFactory.create(config)
    ...
    → GolemAgent(provider, effectiveModel, ...)
```
...
**Impossible without API calls.** Golem/TramAI has no model discovery mechanism. Models are arbitrary strings passed to the provider API. Options:
...
```
User: /provider bogus
  → GolemFactory.create() → ProviderResolver.resolveFromConfig()
...
| Tool execution failure | Caught in GolemAgent | Retried once, then error sent to LLM |
...
1. Close the entire `GolemInstance` (plugins, memory store, index watchers, observers)
2. Create a brand new one from scratch

This destroys:
- **Conversation history** — `GolemAgent`'s in-memory `mutableListOf<ChatMessage>` is lost
...
**Fix:** Add `reconfigure(provider: String, model: String)` to `GolemInstance` that swaps the LLM provider/model without touching the rest. GolemAgent should hold a mutable `ModelProvider` reference.
...
|---------|------|---------|--------|
| `golem` (root) | Main.kt | OK | Well-structured picocli setup |
...
- `golem config` — view current config
- `golem doctor` — diagnostic: test API keys, check provider connectivity
- `golem models` — list available models (would need provider API)
- `golem providers` — list 5 known provider types
...
|---|------|--------|---------|-----|
| 1 | `/provider <bad>` crash | 30min | Runner.kt | Wrap `GolemFactory.create()` in try/catch in slash handler |
| 2 | `/clear` is a no-op | 30min | Runner.kt, GolemAgent.kt | Add `clearConversation()` method, call it |
...
| # | Item | Effort | Description |
|---|------|--------|-------------|
| 16 | Make GolemAgent provider/model mutable | 4h | Add `reconfigure()` method |
...
| 19 | Add `golem doctor` command | 3h | Test API keys, provider connectivity |
...
| 21 | Add `golem config` command | 2h | View current effective config |
| 22 | Implement `/session <id>` properly | 4h | Load from CheckpointStore |
| 23 | Add `golem providers` subcommand | 1h | CLI-level listing |
| 24 | Add `golem models` subcommand | 2h | CLI-level listing (static) |

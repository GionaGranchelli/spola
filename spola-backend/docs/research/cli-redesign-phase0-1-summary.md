...
> **Project:** Golem — Kotlin/JVM multi-module Gradle project  
> **Modules:** `golem-core`, `golem-cli`  
> **Date:** 2026-05-15
...
Main.kt (--resume flag)
  → GolemCli.sessionId
    → buildConfig()
      → GolemConfig(sessionId = ...)        # line 85 of GolemConfig.kt
        → GolemFactory.create(config)
          → AgentFactory.create()
            → GolemAgent(config, checkpointManager)
              → agent.run(persona, goal, observer, sessionId)
                ...
                → checkpointManager.save(sessionId, ...)           # each turn
                  → GolemInstance.run(goal)
                    → calls agent.run(persona, goal, observer, config.sessionId)
```

Key files involved:
- `golem-cli/.../Main.kt` — `--resume` / `--session-id` option (line 153), passed to `buildConfig()` (line 237)
- `golem-core/.../GolemConfig.kt` — `sessionId: String? = null` field (line 84)
- `golem-core/.../GolemAgent.kt` — `run()` accepts `sessionId`, loads/saves checkpoints (lines 52–69, 147–150)
- `golem-core/.../GolemInstance.kt` — `run(goal)` delegates to `agent.run(..., config.sessionId)` (line 26)

### TASK-009: Resource Leak Fix (One Change)

**File:** `golem-core/.../GolemInstance.kt`

**Change:** Added `golemTracer?.close()` to `close()` method (line 36).
...
fun close() {
    ...
    // golemTracer was never closed — leaked OTLP exporter
}
```
...
fun close() {
    ...
    schedulerStore?.close()
    jvmIndexCoordinator?.close()
    golemTracer?.close()   // ← added
}
```

**Why:** `GolemTracer` implements `AutoCloseable` with an OTLP exporter that holds
network resources. Every `/model` or `/provider` switch created a new `GolemTracer`
...
#### 1. Missing `try` keyword in `GolemAgent.runLoop()`

**File:** `golem-core/.../GolemAgent.kt`
...
**File:** `golem-core/.../GolemAgent.kt`, line ~191
...
## Phase 1 — REPL Migration: golem-core → golem-cli

### Architecture Change

**Before (Phase 0, all in `golem-core`):**

| Responsibility | Location |
|---|---|
| `runOneShot()` | `Runner.kt` in `golem-core` |
| `runRepl()` | `Runner.kt` in `golem-core` |
| `ConsoleObserver` | `Runner.kt` in `golem-core` |
| `printHelp()`, `printTools()`, `printMemory()`, `printHistory()` | `Runner.kt` in `golem-core` |
...
| Responsibility | Location |
|---|---|
| `runOneShot()` | `Runner.kt` in `golem-core` (15 lines, minimal) |
| `runRepl()` | `ReplEngine.kt` in `golem-cli` |
| `ReplSession` | `ReplEngine.kt` in `golem-cli` |
| `ConsoleObserver` | `ReplEngine.kt` in `golem-cli` |
| Slash commands | `SlashCommand.kt` in `golem-cli` (12 implementations + registry) |

### New Files

#### 1. `golem-cli/.../SlashCommand.kt`
...
- Implements `AgentRunObserver` interface (stays in `golem-core`)
...
│  - instance: GolemInstance          │  ← replaceable via /model, /provider
│  - turnNumber: Int                  │
│─────────────────────────────────────│
...
1. **Conversation survives model/provider switches** — transcript is held by `ReplSession`, not by `GolemInstance`
...
```
golem-cli → golem-core    (correct direction, no dependency cycle)
```

- `Runner.kt` stays in `golem-core` (only `runOneShot`) since `golem-cli` imports it for `--resume` mode
- `AgentRunObserver` interface stays in `golem-core` (referenced by both modules)
- No remaining references to `ConsoleObserver` or `runRepl` in `golem-core`

---

## Verification Checklist

- [x] `./gradlew :golem-core:compileKotlin` passes
- [x] `./gradlew :golem-cli:compileKotlin` passes
- [x] `./gradlew :golem-core:test` passes (333+ tests)
- [x] `./gradlew :golem-cli:test` passes
- [x] No references to `ConsoleObserver` in `golem-core/` source tree
- [x] No references to `runRepl` in `golem-core/` source tree
...
1. **golem-cli/build.gradle.kts** — Added JLine dependencies:
   - `org.jline:jline:3.27.1`
   - `org.jline:jline-terminal-jna:3.27.1`

2. **golem-cli/.../ReplEngine.kt** — Replaced `console.readLine()` with JLine LineReader:
   - History file at `~/.golem/repl-history.txt`
...
3. **golem-cli/.../JLineCompleter.kt** — NEW (63 lines):
...
- golem-core/* — zero changes
- golem-cli/SlashCommand.kt — unchanged
- golem-cli/Main.kt — unchanged
...
1. **GolemAgent.kt** (core): `provider: ModelProvider` and `effectiveModel: String` changed from `val` to `var`. Added `reconfigure(newProvider, newModel)` method that swaps them in-place.

2. **GolemInstance.kt** (core): `config: GolemConfig` changed from `val` to `var`. Added `reconfigure(providerName, modelName)` that:
...
5. **SlashCommand.kt** (cli): ModelCommand and ProviderCommand now call `session.instance.reconfigure()` instead of `GolemFactory.create()` + `session.replaceInstance()`.
...
1. **GolemAgent.kt** (core): Added `getCheckpointManager()` public getter (was private before).
...
2. **`golem doctor`** — diagnostic subcommand
3. **`golem config`** — view/edit config subcommand

### Implementation

#### A. Verbosity Control

- **`GolemConfig.kt`** — Added `Verbosity` enum (`NORMAL`, `VERBOSE`, `DEBUG`) and `verbosity: Verbosity = Verbosity.NORMAL` field
- **`Main.kt`** — Added `--verbose` and `--debug` boolean options to `GolemCli`; `buildConfig()` computes verbosity from flags (debug → DEBUG, verbose → VERBOSE, else from file config → NORMAL)
...
- **`DoctorCommand.kt`** (new file, 193 lines) — picocli subcommand under `golem`:
...
- **`ConfigCommand.kt`** (new file, 150 lines) — picocli subcommand with 4 sub-subcommands:
  - `golem config show` — prints effective config as YAML (file + CLI overlay)
  - `golem config path` — prints config file path
  - `golem config set <key> <value>` — writes to `~/.golem/config.yaml` with known-key validation
  - `golem config edit` — opens config in `$EDITOR` (fallback: nano → vi)
- **`GolemConfigFileStore.kt`** — Extended with `loadRaw()`, `saveRaw()`, `toYaml()`, `fromRaw()` for YAML round-tripping; added `FAIL_ON_UNKNOWN_PROPERTIES = false` for forward compat
...
This means `golem --model gpt-4o-mini` overrides the file config model, while `golem` without --model uses the file config's model.

#### E. Minor Fixes

- `GolemInstance.reconfigure()` — Now uses agent's existing `CheckpointManager` instead of creating a redundant one (Gemini finding)
- `Runner.kt` (golem-core) — Stripped to only contain `runOneShot`; old REPL code and `ConsoleObserver` were moved to `golem-cli` in Phase 1
- `GolemAgent.kt` — Added `runFull()`, `reconfigure()`, `getCheckpointManager()`, unified `runLoop()`, the `llm_request` status hook for DEBUG mode
...
4. **Config set preserves existing keys**: Loads raw map → modifies single key → converts via GolemConfig → saves. Unknown keys are rejected.
5. **Redundant CheckpointManager removed**: `GolemInstance.reconfigure()` now passes the agent's existing `CheckpointManager` to `rebuildModelDependentTools()`.

# Codex Review: Golem CLI Redesign Plan (v1)

**Reviewer:** OpenAI Codex CLI (gpt-5.4)
**Plan reviewed:** `.hermes/plans/2026-05-14_golem-cli-redesign.md` (v1)
**Date:** 2026-05-14
...
### HIGH — Module cycle
**v1 plan:** Put `ReplEngine`, `SlashCommand`, terminal classes in golem-cli, but keep `runRepl()` in core with delegation to CLI classes.
**Problem:** `golem-cli` depends on `golem-core` (build.gradle.kts:15), not the reverse. Core cannot import CLI classes.
**Fix (applied):** Move `runRepl()` entirely into golem-cli. Core keeps only `runOneShot()`.

### HIGH — Conversation wipes on every run
**v1 plan:** Assumed REPL is multi-turn, but `GolemAgent.run()` at GolemAgent.kt:54 calls `conversation.clear()` at the start of every goal.
...
**Fix (applied):** `ReplSession` abstraction lives outside the agent in golem-cli. `GolemAgent.runFull(persona, goal, preloadedConversation)` skips `clear()` and uses the provided list. Agent's internal conversation is deprecated in REPL mode.

### HIGH — `--resume` is parsed but inert
**v1 plan:** Phase 4 introduces new session machinery.
**Problem:** `Main.kt:153` parses `--resume`, `GolemConfig.kt:65` stores `sessionId`, but `GolemInstance.kt:26` calls `agent.run(persona, goal, observer)` without passing any session id. The feature exists in config but is never threaded into execution.
...
**v1 plan:** Only changes `GolemAgent.provider` and `GolemAgent.effectiveModel`.
...
**Fix (applied):** `reconfigure()` also calls `ToolRegistryFactory.rebuildModelDependentTools(model)`. Provenance tools use dynamic lookup from GolemInstance.

## Medium Findings

### Medium — Resource leak on instance recreation
**Problem:** `GolemInstance.close()` only shuts down plugins and closes memory. It does NOT close the file watcher (`JvmIndexCoordinator`), scheduler store (`SqliteGolemJobStore`), or checkpoint store (`CheckpointStore`). Every `/model` or `/provider` switch leaks these.
**Fix (applied):** `GolemInstance.close()` now owns and closes all resources (TASK-009).

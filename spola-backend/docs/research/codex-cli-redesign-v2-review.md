# Codex Review: Golem CLI Redesign Plan (v2)

**Reviewer:** OpenAI Codex CLI (gpt-5.4)
**Plan reviewed:** `.hermes/plans/2026-05-14_golem-cli-redesign.md` (v2 — revised)
**Date:** 2026-05-14
...
|---|---------|--------|----------|
| HIGH #1 — Module cycle | **RESOLVED** | `runRepl()` moves entirely into golem-cli. Runner.kt keeps only `runOneShot()`. Core → CLI dependency direction is correct. |
...
| HIGH #3 — Dead `--resume` | **RESOLVED** | Phase 0/TASK-008 wires `config.sessionId` into execution. `GolemAgent.run(..., sessionId)` already exists — only `GolemInstance.run()` needed the forwarding. |
...
| MEDIUM #3 — Dynamic model lookup from GolemInstance doesn't fit construction order | Replaced with explicit `ToolRegistryFactory.rebuildModelDependentTools(model)` — unregisters and re-registers provenance tools with the new model string. |
...
- `ConsoleObserver` to be redefined in golem-cli (private in Runner.kt, needs to move)
- `GolemInstance.close()` must own all resources: file watcher, scheduler store, checkpoint store

## Architecture Fit

The module boundary is now correct: terminal and REPL concerns in `golem-cli`, agent/session/provider APIs in `golem-core`.

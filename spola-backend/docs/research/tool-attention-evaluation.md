# Tool Attention — Evaluation for Golem Integration
...
**Status:** Nice-to-have — viable concept, but marginal gains on Golem's current tool surface
...
## Golem's Actual Baseline (measured from source)

Codex audited every registered tool in the codebase (`Tool.kt`, `GolemAgent.kt`, `ToolRegistryFactory.kt`, `SkillTools.kt`, `McpClientManager.kt`):
...
**Why the gap:** The paper's headline savings come from a simulated 120-tool benchmark. Golem has ~52 tools, and many are simple (1-2 string parameters). Tool schemas in Golem are compact by design.
...
The plan assumed `sideEffectLevel`, capability labels, and auth scopes for the state gate. Golem's `Tool` data class only has:
```kotlin
data class Tool(
    val name: String,
...
Golem's tool registry changes at runtime:
...
The plan assumed a "summary pool in a cached prompt prefix." Golem's `ModelRequest` only has `messages` + `tools`. There is no provider-neutral prompt-cache control, and `ConcurrentHashMap` iteration (used by `ToolRegistry.schemas()`) provides non-deterministic tool ordering — bad for stable prefix caching.

### MEDIUM — Duplicates existing gating

`ToolRegistryFactory` already filters tools by agent scope (`networkAccess`, etc.) before tools reach `GolemAgent`. The Tool Attention state gate would re-do this filtering at a different layer.
...
**Viable but low priority.** The paper's central insight (semantic tool routing is better than bulk injection) is sound, but Golem's tool surface is too small for the paper's headline savings. The marginal gains (~40% first-turn) don't justify the complexity at current scale.

**Revisit when:**
- Golem's tool surface grows past 80-100 tools (from skills, MCP servers, plugins)

...
Golem's Kanban system provides simple, SQLite-backed task management. Tasks can be created, updated, listed, and deleted via tools that the agent can call directly — no external service needed.
...
The kanban system is exposed to the Golem agent via four tools registered by `registerKanbanTools()`.

...
@Command(name = "set", description = ["Set a top-level config value in ~/.golem/config.yaml"])
...
class ConfigEditCommand : Callable<Int> {
    ...
    private fun ensureConfigFileExists(path: Path) {
        ...
        CliConfigSupport.store.save(GolemConfig())
    }
...
internal object CliConfigSupport {
    val store = GolemConfigFileStore()
    ...
    fun toConfig(raw: Map<String, Any?>): GolemConfig {

...
class DoctorCommand : Callable<Int> {
    ...
    override fun call(): Int = runBlocking {
        ...
        val config = effectiveConfig.getOrDefault(GolemConfig())
        ...
        println("${ANSI_BOLD}Golem doctor${ANSI_RESET}")
    ...
    private fun databaseChecks(config: GolemConfig): List<DoctorResult> {
        return listOf(
    ...
    private fun environmentCheck(config: GolemConfig): DoctorResult {
        ...
        val message = when (provider) {
            ...
            "ollama" -> "No API key required"
                ...
                if (config.customProviders.any { it.name.lowercase() == provider }) {
                    singleEnvCheck("GOLEM_API_KEY") + " (custom provider '$provider' may use its own apiKey field)"
                } else {
    ...
    private suspend fun providerConnectivityCheck(config: GolemConfig, configValid: Boolean): DoctorResult {
        if (!configValid) {
    ...
    private fun personaCheck(config: GolemConfig): DoctorResult {
        return try {
    ...
    private fun skillsDirectoryCheck(config: GolemConfig): DoctorResult {
        return try {

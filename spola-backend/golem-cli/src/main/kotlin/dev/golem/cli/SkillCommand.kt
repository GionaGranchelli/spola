...
class SkillListCommand : Callable<Int> {
    ...
    override fun call(): Int {
        ...
        if (skills.isEmpty()) {
            ...
            println("Place .yaml files in $skillsDir or use 'golem skill install <path>'")
            return 0
        }
 ...
 */
private fun skillsDirectory(root: GolemCli): Path {
    val configured = buildConfig(root).skillsDir
    return if (configured.isBlank()) {
        Path.of(System.getProperty("user.home"), ".golem", "skills")
    } else {
        Path.of(configured)
    }
}

...
@Command(
    name = "workflow",
    description = ["Run and manage Golem workflows"],
    subcommands = [
...
class TeamRunCommand : Callable<Int> {
    ...
    override fun call(): Int = runBlocking {
        ...
        val config = buildConfig(root)
        val wf = workflow<GolemState>("team-run", "1") {
            parallelAgentsStep(
                name = "team",
                agents = agentIds,
                goal = { it.goal },
        ...
        val result = wf.run(initialState = GolemState.initial(goal = goal, config = config))
        println(result)
        0
    }
}

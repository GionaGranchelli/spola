...
/**
 * Creates and manages OpenTelemetry tracing spans for Golem agent runs.
 *
...
class GolemTracer(
    ...
    companion object {
        ...
        fun noop(): GolemTracer = GolemTracer(
            otelEnabled = false,
            otelEndpoint = null,
        )
    }
    ...
    fun startRootSpan() {
        val span = tracer.spanBuilder("golem.run")
            .setAttribute("golem.service.name", "golem")
            .startSpan()
    ...
    fun startTurnSpan(turn: Int) {
        ...
        val span = tracer.spanBuilder("golem.turn")
            .setParent(Context.current().with(root))
            .setAttribute("golem.turn.number", turn.toLong())
            .startSpan()
    ...
    fun startLlmCallSpan(model: String, provider: String) {
        ...
        val span = tracer.spanBuilder("golem.llm.call")
    ...
    fun startToolSpan(toolName: String, toolCallId: String) {
        val turnSpan = findCurrentTurnSpan()
        val parent = turnSpan ?: activeSpans["root"] ?: Span.getInvalid()
        val span = tracer.spanBuilder("golem.tool.execution")
            .setParent(Context.current().with(parent))
            .setAttribute("golem.tool.name", toolName)
            .setAttribute("golem.tool.call_id", toolCallId)
            .startSpan()
    ...
    fun endToolSpan(toolCallId: String, success: Boolean, durationMs: Long? = null) {
        ...
        if (span != null) {
            span.setAttribute("golem.tool.success", success)
            durationMs?.let { span.setAttribute("golem.tool.duration_ms", it) }
            if (!success) {

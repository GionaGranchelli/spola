...
/**
 * MCP (Model Context Protocol) server that exposes all Golem tools as MCP tools.
 *
 * Any MCP client (Hermes, Claude Code, Codex CLI) can discover and call Golem's
 * built-in tools through stdio or SSE transport.
 */
class GolemMcpServer(
    private val toolRegistry: ToolRegistry,
    private val serverName: String = "golem-mcp",
    ...
    fun buildServer(): Server {
        ...
        // Register each Golem tool as an MCP tool
        for (golemTool in toolRegistry.list()) {
            mcpServer.addTool(
                tool = McpTool(
                    name = golemTool.name,
                    description = golemTool.description,
                    inputSchema = buildToolSchema(golemTool),
                ),
            ) { request ->
                val result = executeGolemTool(golemTool, request.arguments)
                CallToolResult(
    ...
    /**
     * Execute a Golem tool with properly decoded arguments.
     * Handles the MCP SDK's JsonElement → typed values conversion.
     */
    private suspend fun executeGolemTool(
        tool: Tool,
    ...
    /**
     * Convert Golem tool parameters to MCP JSON Schema.
     */
    private fun buildToolSchema(golemTool: Tool): ToolSchema {
        val properties = buildJsonObject {
            for (param in golemTool.parameters) {

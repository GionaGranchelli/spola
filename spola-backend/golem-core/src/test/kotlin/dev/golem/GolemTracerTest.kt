...
class GolemTracerTest {
    ...
    fun `noop constructor via config is noop`() {
        val tracer = GolemTracer(otelEnabled = false, otelEndpoint = null)
        assertThat(tracer.isActive).isFalse()

        val tracer2 = GolemTracer(otelEnabled = true, otelEndpoint = null)
        assertThat(tracer2.isActive).isFalse()

        val tracer3 = GolemTracer(otelEnabled = false, otelEndpoint = "http://localhost:4317")
    ...
    fun `active tracer creates spans with correct hierarchy`() {
        // Use SDK directly to test span creation behavior matching GolemTracer's pattern
        ...
        val otel = OpenTelemetrySdk.builder()
            ...
            .build()
        val tracer = otel.getTracer("golem-test")

        // Create root span (like GolemTracer.startRootSpan)
        val rootSpan = tracer.spanBuilder("golem.run")
            .setAttribute("golem.goal", "test goal")
            .startSpan()

        // Create turn span as child (like GolemTracer.startTurnSpan)
        val turnSpan = tracer.spanBuilder("golem.turn")
            .setParent(Context.current().with(rootSpan))
            .setAttribute("golem.turn.number", 1L)
            .startSpan()

        // Create LLM call span as child of turn (like GolemTracer.startLlmCallSpan)
        val llmSpan = tracer.spanBuilder("golem.llm.call")
        ...
        // Create tool span as child of turn (like GolemTracer.startToolSpan)
        val toolSpan = tracer.spanBuilder("golem.tool.execution")
            .setParent(Context.current().with(turnSpan))
            .setAttribute("golem.tool.name", "read_file")
            .setAttribute("golem.tool.call_id", "call-1")
            .startSpan()
        toolSpan.setAttribute("golem.tool.success", true)
        toolSpan.setAttribute("golem.tool.duration_ms", 42L)
        toolSpan.end()
        ...
        // Find spans by name
        val root = spans.find { it.name == "golem.run" }
        val turn = spans.find { it.name == "golem.turn" }
        val llm = spans.find { it.name == "golem.llm.call" }
        val tool = spans.find { it.name == "golem.tool.execution" }
        ...
        assertThat(root!!.name).isEqualTo("golem.run")
        ...
        assertThat(tool!!.attributes.get(io.opentelemetry.api.common.AttributeKey.stringKey("golem.tool.name")))
            .isEqualTo("read_file")
        assertThat(tool.attributes.get(io.opentelemetry.api.common.AttributeKey.booleanKey("golem.tool.success")))
            .isTrue()
    ...
    fun `error spans record exceptions and error status`() {
        ...
        // Create root span
        val rootSpan = tracer.spanBuilder("golem.run")
            .setAttribute("golem.goal", "test")
            .startSpan()

        // Create tool span that fails
        val toolSpan = tracer.spanBuilder("golem.tool.execution")
            .setParent(Context.current().with(rootSpan))
            .setAttribute("golem.tool.name", "failing_tool")
            .startSpan()
        toolSpan.setAttribute("golem.tool.success", false)
    ...
    fun `config creates correctly configured tracer`() {
        ...
        val defaultConfig = GolemConfig()
        assertThat(defaultConfig.otelEnabled).isFalse()
        assertThat(defaultConfig.otelEndpoint).isNull()
        assertThat(defaultConfig.otelServiceName).isEqualTo("golem")
    }

    // ── TEST 6: GolemTracerObserver span lifecycle ───────────────
    ...
    fun `tracer observer creates correct span lifecycle via SDK`() {
        ...
        // Replicate what GolemTracerObserver does via the SDK directly
        // Start: onStatus("started")
        val rootSpan = tracer.spanBuilder("golem.run")
            .setAttribute("golem.goal", "Agent run started")
            .startSpan()

        // Turn 1: onStatus("thinking")
        val turn1Span = tracer.spanBuilder("golem.turn")
            .setParent(Context.current().with(rootSpan))
            .setAttribute("golem.turn.number", 1L)
            .startSpan()
        ...
        val toolSpan = tracer.spanBuilder("golem.tool.execution")
            .setParent(Context.current().with(turn1Span))
            .setAttribute("golem.tool.name", "echo")
            .setAttribute("golem.tool.call_id", "call-1")
            .startSpan()
        toolSpan.setAttribute("golem.tool.success", true)
        toolSpan.setAttribute("golem.tool.duration_ms", 15L)
        toolSpan.end()
        ...
        // Verify hierarchy
        val rootSpanItem = spans.find { it.name == "golem.run" }
        val turnSpanItem = spans.find { it.name == "golem.turn" }
        val toolSpanItem = spans.find { it.name == "golem.tool.execution" }
        val llmSpanItem = spans.find { it.name == "golem.llm.call" }

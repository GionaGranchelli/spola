...
class GolemApiServerTest {
    ...
    fun `health endpoint works without auth`() = testApplication {
        ...
        application {
            golemApiModule(
                agentRunHandler = AgentRunHandler(),
                jobStore = store,
                config = GolemConfig(apiKey = "secret"),
            )
        }
    ...
    fun `config endpoint returns frontend shaped config response`(@TempDir tempDir: Path) = testApplication {
        val store = SqliteGolemJobStore(tempDir.resolve("scheduler.db").toString())
        val configFileStore = GolemConfigFileStore(tempDir.resolve("config.yaml"))
        application {
            golemApiModule(
                agentRunHandler = AgentRunHandler(),
                jobStore = store,
    ...
    fun `config save endpoint writes yaml and preserves omitted secrets`(@TempDir tempDir: Path) = testApplication {
        ...
        val configFileStore = GolemConfigFileStore(configPath)
    ...
    fun `agent run endpoint executes agent and returns result`(@TempDir tempDir: Path) = testApplication {
        ...
        val store = SqliteGolemJobStore(tempDir.resolve("jobs.db").toString())
    ...
    fun `memory endpoint lists saved entries and supports search`(@TempDir tempDir: Path) = testApplication {
        ...
        runBlocking {
            ...
            memStore.save("project:name", "Golem")
        }
        ...
        assertTrue(listBody.contains("Golem"), "Should contain Golem: $listBody")
    ...
    fun `delivery telegram endpoint requires telegram tool`(@TempDir tempDir: Path) = testApplication {
        ...
        application {
            golemApiModule(
                ...
                config = GolemConfig(apiKey = "test-key"),
            )
        }
    ...
    fun `metrics endpoint returns prometheus text`() = testApplication {
        val store = SqliteGolemJobStore(":memory:")
        val metrics = dev.spola.metrics.GolemMetrics(isEnabled = false)
        application {
            golemApiModule(
                agentRunHandler = AgentRunHandler(),
                jobStore = store,

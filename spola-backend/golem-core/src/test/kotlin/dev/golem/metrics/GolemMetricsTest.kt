...
class GolemMetricsTest {
    ...
    fun `counter increments correctly`() {
        ...
        val agentRuns = families.firstOrNull { it.name == "golem_agent_runs" }
        assertTrue(agentRuns != null, "Should find golem_agent_runs. Found: ${families.map { it.name }}")
    ...
    fun `counter increments with different labels`() {
        ...
        val families = collectFamilies(registry)
        val agentRuns = families.first { it.name == "golem_agent_runs" }
        val nonCreated = agentRuns.samples.filter { !it.name.endsWith("_created") }
    ...
    fun `histogram records observations`() {
        ...
        val families = collectFamilies(registry)
        val agentDuration = families.first { it.name == "golem_agent_run_duration_seconds" }
    ...
    fun `tool calls are recorded`() {
        ...
        val families = collectFamilies(registry)
        val toolCalls = families.first { it.name == "golem_tool_calls" }
    ...
    fun `llm calls are recorded`() {
        ...
        val families = collectFamilies(registry)
        val llmCalls = families.first { it.name == "golem_llm_calls" }
    ...
    fun `tokens are recorded`() {
        ...
        val families = collectFamilies(registry)
        val tokens = families.first { it.name == "golem_llm_tokens" }
    ...
    fun `gauge tracks active sessions`() {
        ...
        val families = collectFamilies(registry)
        val gauge = families.first { it.name == "golem_active_sessions" }
    ...
    fun `scheduler jobs are recorded`() {
        ...
        val families = collectFamilies(registry)
        val jobs = families.first { it.name == "golem_scheduler_jobs_executed" }
    ...
    fun `disabled metrics are no-ops`() {
        val registry = io.prometheus.client.CollectorRegistry()
        val metrics = GolemMetrics(registry = registry, isEnabled = false)
    ...
    fun `renderPrometheusText returns valid format`() {
        ...
        assertTrue(text.contains("golem_agent_runs"))
        assertTrue(text.contains("golem_agent_turns"))
        assertTrue(text.contains("golem_llm_calls"))
    ...
    fun `turn counter increments`() {
        ...
        val families = collectFamilies(registry)
        val turns = families.first { it.name == "golem_agent_turns" }

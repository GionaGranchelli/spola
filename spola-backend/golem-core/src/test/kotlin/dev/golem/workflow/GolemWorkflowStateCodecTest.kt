...
class GolemWorkflowStateCodecTest {

    private val codec: WorkflowStateCodec<GolemState> = GolemWorkflowStateCodec()

    // -- encode/decode round-trip tests -----------------------------------------------

    @Test
    fun `encode and decode round-trips a GolemState`() {
        val original = GolemState(
            goal = "test-goal",
            config = GolemConfig(),
    ...
    @Test
    fun `encode round-trips a minimal GolemState`() {
        val original = GolemState(goal = "minimal")
        val encoded = codec.encode(original)
        val decoded = codec.decode(encoded)
        assertEquals(original.goal, decoded.goal)
    ...
    @Test
    fun `encode round-trips a GolemState with null result`() {
        val original = GolemState(
            goal = "no-result-yet",
            turnCount = 1,
        )
    ...
    fun `encode produces parseable JSON for empty state`() {
        val state = GolemState(goal = "empty-state")
        val json = codec.encode(state)
    ...
    @Test
    fun `configurePersistence defaults to GolemWorkflowStateCodec`() {
        val persistence = GolemFactory.configurePersistence()
        assertIs<GolemWorkflowStateCodec>(persistence.stateCodec)
    }
    ...
    fun `runWorkflow with persistence performs checkpointing`() = runTest {
        val initialState = GolemState(goal = "persistence-test")

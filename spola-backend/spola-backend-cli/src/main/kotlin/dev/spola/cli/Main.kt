...
import dev.spola.scheduler.SpolaScheduler
...
class SpolaCli : Callable<Int> {
    ...
    @Option(
        ...
        defaultValue = "./.spola/memory.db",
    )
    var memoryDb: String = "./.spola/memory.db"
    ...
    @Option(
        ...
        defaultValue = "./.spola/scheduler.db",
    )
    var schedulerDb: String = "./.spola/scheduler.db"

    @Option(
        names = ["--kanban-db"],
        description = ["Path to SQLite kanban database"],
        defaultValue = "./.spola/kanban.db",
    )
    var kanbanDb: String = "./.spola/kanban.db"

    @Option(
        names = ["--jvm-index-db"],
        description = ["Path to SQLite JVM project index database"],
        defaultValue = "./.spola/jvm-index.db",
    )
    var jvmIndexDb: String = "./.spola/jvm-index.db"
...
suspend fun <T> withAgentStore(root: SpolaCli, block: suspend (dev.spola.agent.AgentStore) -> T): T {
...
fun buildConfig(root: SpolaCli): SpolaConfig {
    val baseConfig = SpolaConfigFileStore().load()
...
suspend fun <T> withJobStore(root: SpolaCli, block: suspend (SpolaJobStore) -> T): T {
    val store = SqliteSpolaJobStore(root.schedulerDb)
    return try {
        block(store)
    } finally {
        store.close()
...
fun main(args: Array<String>) {
    CommandLine(SpolaCli()).execute(*args)
}

private fun SpolaCli.optionMatched(vararg names: String): Boolean {
...
private fun <T> SpolaCli.overrideIfMatched(name: String, cliValue: T, baseValue: T): T {
    return if (optionMatched(name)) cliValue else baseValue
}

private fun <T> SpolaCli.overrideIfMatched(names: Array<String>, cliValue: T, baseValue: T): T {

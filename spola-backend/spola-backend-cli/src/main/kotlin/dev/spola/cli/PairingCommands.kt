...
@Command(
    name = "pairing",
    description = ["Manage Spola pairing configuration"],
...
private fun buildPairingCliInfo(root: SpolaCli): PairingCliInfo {
    val resolvedApiKey = root.apiKey ?: System.getenv("SPOLA_API_KEY") ?: System.getenv("GOLEM_API_KEY")
...
class PairingInfoCommand : Callable<Int> {
    ...
    override fun call(): Int {
        ...
        println(jsonText)
        println()
        println("Or run 'spola pairing qrcode' to generate a QR code image.")
        return 0
    }
}

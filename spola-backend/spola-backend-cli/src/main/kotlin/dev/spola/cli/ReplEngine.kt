 ...
 *
 * Unlike [SpolaAgent.run], which clears and rebuilds the conversation on
 ...
 */
class ReplSession(
    /** The active Spola agent instance. Mutable for /model and /provider switches. */
    var instance: SpolaInstance,
) {
...
/**
 * Run Spola in REPL mode: interactive loop reading goals, dispatching slash
 * commands, and showing results.
 */
suspend fun runRepl(config: SpolaConfig = SpolaConfig()) {
    var currentConfig = config
    val observer = ConsoleObserver(config.verbosity)
    var session = ReplSession(
        instance = SpolaFactory.create(config = currentConfig, observer = observer),
    )

    println("${ANSI_BOLD}Spola v${SpolaVersion.VERSION}${ANSI_RESET} ${ANSI_DIM}— JVM Autonomous Coding Agent${ANSI_RESET}")
    ...
    if (console == null) {
        ...
        println("     ${ANSI_CYAN}./gradlew :spola-backend-cli:installDist${ANSI_RESET}")
        println("     ${ANSI_CYAN}./spola-backend-cli/build/install/spola/bin/spola${ANSI_RESET}")
        println()
        println("   Or use one-shot mode:")
        println("     ${ANSI_CYAN}./gradlew :spola-backend-cli:run --args=\\\"'your goal here'\\\"${ANSI_RESET}")
        println()
        println("   Or run with -Dorg.gradle.internal.interactive=true (experimental):")
        println("     ${ANSI_CYAN}./gradlew :spola-backend-cli:run --console=plain${ANSI_RESET}")
        session.close()
        return
    }
    ...
    try {
        try {
            val historyFile = Path.of(System.getProperty("user.home"), ".spola", "repl-history.txt")

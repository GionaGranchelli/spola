 ...
 * the full hierarchy (e.g., SpolaCli -> workflow -> run) so picocli
...
class WorkflowCliTest {
    ...
    fun `WorkflowCommand shows help text when invoked`() {
        ...
        assertTrue(text.contains("Run and manage Spola workflows"))
    }
    ...
    fun `WorkflowRunCommand parent is accessible via hierarchy`() {
        ...
        assertTrue(runCmd.workflowCommand.parent is SpolaCli)
    }
    ...
    fun `TeamRunCommand parent is accessible via hierarchy`() {
        ...
        assertTrue(runCmd.teamCommand.parent is SpolaCli)
    }
    ...
    // -----------------------------------------------------------------------
    // SpolaCli top-level — verify workflow and team are registered
    // -----------------------------------------------------------------------

    @Test
    fun `SpolaCli registers workflow and team subcommands`() {
    ...
    @Test
    fun `SpolaCli usage mentions workflow and team`() {
        val out = ByteArrayOutputStream()
    ...
    @Test
    fun `SpolaCli constructs workflow run code-review with wiring`() {
        val spolaCli = SpolaCli()
        val workflowCmd = WorkflowCommand().apply { parent = spolaCli }
        val runCmd = WorkflowRunCommand()
        ...
        assertTrue(runCmd.workflowCommand.parent === spolaCli)
    }

    @Test
    fun `SpolaCli constructs team run with agents and goal wiring`() {
        val spolaCli = SpolaCli()
        val teamCmd = TeamCommand().apply { parent = spolaCli }
        val runCmd = TeamRunCommand()
        ...
        assertTrue(runCmd.teamCommand.parent === spolaCli)
    }
}

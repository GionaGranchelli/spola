 ...
 * GET /api/memory/{key}/html and the `spola memory render <key>` CLI command.
 *
 * Supports:
 * - Bold (**text** → &lt;strong&gt;)
 * - Italic (*text* → &lt;em&gt;)
...
object HtmlMemoryRenderer {
    ...
    fun renderMemoryList(
        ...
        appendLine("<title>Spola Memory" + (query?.let { " — Search: " + sanitizeHtml(it) } ?: "") + "</title>")
        appendLine("<style>$baseCss</style>")
        appendLine("</head>")
        appendLine("<body>")
        appendLine("<div class=\"container\">")
        appendLine("<h1>🧠 Spola Memory</h1>")
        if (query != null) {
    ...
    private fun wrapDocument(body: String): String = buildString {
        ...
        appendLine("<meta charset=\"UTF-8\">")
        appendLine("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">")
        appendLine("<title>Spola Memory</title>")

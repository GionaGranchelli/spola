        ...
        execute = { args ->
            try {
                ...
                if (token == null) return@Tool ToolResult.fail(
                    "Telegram bot token not configured. Set TELEGRAM_BOT_TOKEN env var or configure telegramBotToken in SpolaConfig."
                )
...
private fun emailSendTool(config: SpolaConfig): Tool {
        ...
        execute = { args ->
            try {
                ...
                if (host == null || host.isBlank()) return@Tool ToolResult.fail(
                    "SMTP host not configured. Set EMAIL_SMTP_HOST env var or configure emailSmtpHost in SpolaConfig."
                )
                if (username == null || password == null) return@Tool ToolResult.fail(
                    "SMTP credentials not configured. Set EMAIL_USERNAME and EMAIL_PASSWORD env vars or configure in SpolaConfig."
                )
                if (from == null || from.isBlank()) return@Tool ToolResult.fail(
                    "SMTP from address not configured. Set EMAIL_FROM env var or configure emailFrom in SpolaConfig."
                )

...
object DoneConditionEvaluator {
     ...
     */
    fun evaluateAll(conditions: List<DoneCondition>, state: SpolaState, stepOutputs: Map<String, String>): Boolean {
     ...
     */
    fun evaluate(condition: DoneCondition, state: SpolaState, stepOutputs: Map<String, String>): Boolean {
        val result = try {
    ...
    private fun evaluateAllAgentsCompleted(state: SpolaState): Boolean {
        return state.intermediateResults.isNotEmpty()
    }

    private fun evaluateOutputHasContent(state: SpolaState): Boolean {
        return !state.result.isNullOrBlank()
    }

    private fun evaluateOutputContains(state: SpolaState, regex: String): Boolean {
    ...
    private fun evaluateMarkdownValid(state: SpolaState): Boolean {
        val output = state.result ?: return false
    ...
    private fun evaluateAllStepsPassed(state: SpolaState): Boolean {
        // If we got a result, steps passed
        return state.result != null
    }

    private fun evaluateNoCriticalBlockers(state: SpolaState): Boolean {
    ...
    private fun evaluateReportGenerated(state: SpolaState): Boolean {
        val output = state.result ?: return false

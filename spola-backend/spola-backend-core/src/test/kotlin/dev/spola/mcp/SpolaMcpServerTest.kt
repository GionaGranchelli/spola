...
class SpolaMcpServerTest {
    ...
    fun `tool parameter types map to schema correctly`() {
        ...
        assertNotNull(SpolaMcpServer(toolRegistry = registry).buildServer())
    }
    ...
    fun `read_file tool executes on temp file`() {
        val tempDir = java.nio.file.Files.createTempDirectory("spola-backend-mcp-test-")
        try {

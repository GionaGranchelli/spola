...
class MetricsEndpointTest {
    ...
    fun `metrics endpoint returns valid Prometheus format`() = testApplication {
        ...
        assertTrue(body.contains("spola_agent_runs_total"), "Should contain agent runs metric")
        assertTrue(body.contains("spola_tool_calls_total"), "Should contain tool calls metric")
    ...
    fun `metrics endpoint is accessible without auth`() = testApplication {
        ...
        application {
            spolaApiModule(
                agentRunHandler = AgentRunHandler(),
                jobStore = store,
                config = SpolaConfig(apiKey = "secret-key"),
                spolaMetrics = SpolaMetrics(registry = registry),
            )
        }
    ...
    fun `metrics endpoint returns all metric families when recorded`() = testApplication {
        ...
        assertTrue(body.contains("spola_agent_runs_total"))
        assertTrue(body.contains("spola_agent_run_duration_seconds"))
        assertTrue(body.contains("spola_agent_turns_total"))
        assertTrue(body.contains("spola_tool_calls_total"))
        assertTrue(body.contains("spola_tool_call_duration_seconds"))
        assertTrue(body.contains("spola_llm_calls_total"))
        assertTrue(body.contains("spola_llm_tokens_total"))
        assertTrue(body.contains("spola_scheduler_jobs_executed_total"))
        assertTrue(body.contains("spola_active_sessions"))

...
class TeamWorkflowStepsTest {
    ...
    fun `parallelAgentsStep adds step with correct name to workflow builder`() {
        with(TeamWorkflowSteps) {
            val builder = dev.tramai.orchestration.workflow<SpolaState>("test-parallel") {
                parallelAgentsStep(
                    name = "my-agents",
    ...
    fun `parallelAgentsStep wires agents list and goal function through`() {
        with(TeamWorkflowSteps) {
            val builder = dev.tramai.orchestration.workflow<SpolaState>("test-wiring") {
                parallelAgentsStep(
                    name = "multi-agent",
    ...
    fun `parallelAgentsStep default merge appends to intermediateResults`() {
        with(TeamWorkflowSteps) {
            ...
            val defaultMerge: (SpolaState, List<String>) -> SpolaState = { s, results ->
                s.copy(
    ...
    fun `branchOnResult routes to correct branch`() {
        with(TeamWorkflowSteps) {
            val builder = dev.tramai.orchestration.workflow<SpolaState>("test-branch-routing") {
                branchOnResult(
                    name = "route",
    ...
    fun `branchOnResult uses default branch when no key matches`() {
        with(TeamWorkflowSteps) {
            val builder = dev.tramai.orchestration.workflow<SpolaState>("test-branch-default") {
                branchOnResult(
                    name = "route",
    ...
    fun `humanApprovalGate allows workflow to continue`() {
        with(TeamWorkflowSteps) {
            val builder = dev.tramai.orchestration.workflow<SpolaState>("test-gate-allow") {
                humanApprovalGate(
                    name = "approve",
    ...
    fun `humanApprovalGate rejects workflow`() {
        with(TeamWorkflowSteps) {
            val builder = dev.tramai.orchestration.workflow<SpolaState>("test-gate-reject") {
                humanApprovalGate(
                    name = "reject",

...
open class OpenClawRestGatewayClient(
    ...
    fun chatCompletionsStream(
        ...
        client.preparePost("$baseUrl/v1/chat/completions") {
            ...
            token?.let { header(HttpHeaders.Authorization, "Bearer $it") }
            sessionKey?.let { header("x-openclaw-session-key", it) }
            agentId?.let { header("x-openclaw-agent-id", it) }
            ...
            if (response.status != HttpStatusCode.OK) {
                ...
                throw RuntimeException("OpenClaw API Error (${response.status}): $error")
            }
